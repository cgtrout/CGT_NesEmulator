...
public:
	...
	TFreeList( int iNumObjects )
	 {
		//_ASSERT( ( iNumObjects > 0 ) && "Invalid TFreeList size specified." );

		m_pObjectData = new FLDataType[ iNumObjects ];
		m_ppFreeObjects = new FLDataType*[ iNumObjects ];

		//_ASSERT( m_pObjectData && "Not enough memory to allocate object data!" );
		//_ASSERT( m_ppFreeObjects && "Not enough memory to allocate pointer stack!" );
	...
	TFreeList( FLDataType *pObjectData, FLDataType **ppFreeObjects, int iNumObjects )
	 {
		...
		//_ASSERT( m_pObjectData && "A NULL pointer was passed for the object data!" );
		//_ASSERT( m_ppFreeObjects && "A NULL pointer was passed for the pointer stack!" );
	...
	FLDataType *NewInstance( void )
	 {
		//_ASSERT( m_iTop && "Tried to get a new instance but there"
	...
	void FreeInstance( FLDataType *pInstance )
	 {
		//_ASSERT( ( pInstance >= &( m_pObjectData[ 0 ] ) ) &&
		...
		//_ASSERT( ( m_iTop < m_iNumObjects ) && "You have freed at least one"
